...
const STOP = "(?:год|года|году|страна|страны|комплектаци|пробег|привод|объем|объём|мощност|двигател|коробк|коробка|цвет|цветом|цена|ценой|передни|полный|задни|автомат|механик|вариатор|робот|тысяч|миллион|литр|рубл|за|на)";
...
export function parseTranscript(raw: string): ParsedCar {
  ...
  if (milWin) {
    ...
    const start = t.indexOf(milWin[0]);
    const tail = t.slice(start + milWin[0].length, start + milWin[0].length + 14);
  ...
  if (eDec) out.engine = `${Number(eDec[1])}.${eDec[2].charAt(0)} л`;
    ...
    const ew = eWin.match(/(^|[^а-я])(один|два|три)(?:\s+и\s+(\d))?\s*(?:литра|литров|л\b)?/);
    if (ew && (ew[3] || /литр|\sл\b/.test(eWin))) {
      const base = { один: 1, два: 2, три: 3 }[ew[2] as "один" | "два" | "три"] ?? 2;
      out.engine = ew[3] ? `${base}.${ew[3]} л` : `${base}.0 л`;
    }
  }
  ...
  /* --- тип двигателя --- */
  if (/электро|электр/.test(t)) out.fuel = "Электрический";
