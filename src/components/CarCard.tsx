...
const Spec = ({ icon, label, children }: { icon: React.ReactNode; label: string; children: React.ReactNode }) => (
  <div className="flex items-start gap-2 border-t border-line/80 py-2">
    <span className="mt-0.5 text-ink-3 shrink-0">{icon}</span>
...
export default function CarCard({ car, index, onEdit, onDelete }: Props) {
  ...
  return (
    <article
      className="group relative flex h-full flex-col overflow-hidden rounded-[5px] border-2 border-ink bg-panel shadow-hard-sm transition-all duration-300 hover:-translate-y-1.5 hover:shadow-hard"
    >
      {/* номерной ярлык */}
      <div className="pointer-events-none absolute right-0 top-0 z-20 plate px-2.5 py-1 font-display text-[10px] tracking-[0.18em] text-paper">
      ...
      <div className="relative aspect-[16/10] overflow-hidden border-b-2 border-ink bg-[#dfe3e7]">
        ...
        <div className="absolute bottom-0 left-0 z-10 flex gap-0 border-t-2 border-r-2 border-ink">
      ...
      <footer className="mt-auto flex items-end justify-between gap-3 border-t-2 border-ink bg-paper px-4 py-3">
        ...
        {confirm ? (
          <div className="flex shrink-0 items-center gap-1.5 rounded-[4px] border-2 border-ink bg-paper px-2 py-1.5 anim-pop">
